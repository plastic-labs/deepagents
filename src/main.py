...
class TaskAgent:
    def __init__(self, tools: List[str], instructions: str, subagents: List[SubAgent]):
        self.agents = {}
    ...
    def setup_agents(
        ...
        self.agents["general-purpose"] = Agent(tools, instructions)

        # Custom subagents
        for subagent in subagents:
            agent_tools = subagent.tools if subagent.tools else tools
            self.agents[subagent.name] = Agent(agent_tools, subagent.prompt)
...
def create_deep_agent(
    ...
    # Add built-in tools
    all_tools.extend([write_todos, ls, read_file, write_file, edit_file])
    
    # Add user-provided tools
    all_tools.extend(tools)
    
    return Agent(all_tools, instructions, model)
